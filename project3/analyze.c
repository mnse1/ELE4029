    ...
    case StmtK:
        ...
        case FuncK:
            if (st_lookup(t->attr.name) == -1)
            {
                ...
                if (t->kind.stmt == FuncK)
                    enterScope();
            }
            else
            {
                typeError(t, "Symbol redefinition");
            }
            break;
...
void buildSymtab(TreeNode *syntaxTree)
{
    ...
    st_insert("input", 0, location++, Integer);  // Built-in function input()
    st_insert("output", 0, location++, Void);    // Built-in function output()
    traverse(syntaxTree, insertNode, nullProc);
